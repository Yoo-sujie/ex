...
public class OutView extends JPanel {
	...
	public OutView() {
		...
		btnDeUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				String sellCode = labelDeCode.getText();
				...
				else if(tfDeCustomerName.getText().equals("")) {
				...
				try {
					resultAndQuantity = new OutViewDAO().updateSellTable(sellCode, quantity, pdCode, pdName, date, info, total);
					if(resultAndQuantity[1] > 0) {
						JOptionPane.showMessageDialog(null, sellCode + "의 내용이 수정되었습니다", "수정 완료",JOptionPane.INFORMATION_MESSAGE);
						VOSellProduct sellPd = new VOSellProduct(sellCode, lblDeCustomerCode.getText(), pdCode, quantity, Integer.parseInt(lblTotalMoney.getText()),date, info);
						updateTable(sellPd, tableselectedRow);
						clearField();
					}
					else {
						JOptionPane.showMessageDialog(null, sellCode + "의 내용이 수정이 실패 했습니다", "수정 에러",JOptionPane.INFORMATION_MESSAGE);
						return;
					}
				...
				try {
					int result = new OutViewDAO().reflectStockToUpdate(pdCode, quantity, resultAndQuantity[0]);
		...
		btnDeDelete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				int result = 0;
				try {
					result = new OutViewDAO().deleteSell(sellCode);
				...
				if(result > 0) {
					JOptionPane.showMessageDialog(null, sellCode + "이 삭제되었습니다", "삭제 완료",JOptionPane.INFORMATION_MESSAGE);
					((DefaultTableModel)tableDeList.getModel()).removeRow(tableselectedRow);
					clearField();
				}
				else {
					JOptionPane.showMessageDialog(null, sellCode + "의 삭제에 실패했습니다.", "삭제 살패",JOptionPane.INFORMATION_MESSAGE);
				}
			}
		});
		...
		lblTotalMoney.setBounds(102, 219, 177, 29);
		...
		btnDeSearchAll.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(orderValue.equals("코드")) orderValue = "sell_code";
				else if(orderValue.equals("수량")) orderValue = "sell_quantity";
				else if(orderValue.equals("총액")) orderValue = "SELL_amount";
				else orderValue = "output_date";


				try {
					//	전체 조회 쿼리, 판매 목록 리스트 반환
					ArrayList<VOSellProduct> sellpdlist = new OutViewDAO().selectAll(orderBy, orderValue);
					//	테이블 세팅 메서드
					setTable(sellpdlist);
					return;
				} catch (ClassNotFoundException e) {
		...
		String[] sellcol = new String[] { "판매코드","거래품목","거래처","수량","총액","날짜","정보"};
		...
		tableDeList.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				try {
					String pdName = new OutViewDAO().getPdName((String)tableDeList.getValueAt(tableselectedRow, 1));
					String ctmName = new OutViewDAO().getCtmName((String)tableDeList.getValueAt(tableselectedRow, 2));
		...
		btnDeSelectIndex.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				ArrayList<VOSellProduct> sellpdlist;
				...
				try {
					sellpdlist = new OutViewDAO().searchSelected(target, searchText, orderBy, orderValue);
					setTable(sellpdlist);
					return;
	...
	public void setTable(ArrayList<VOSellProduct> sellpdlist) {
		sellListModel.setRowCount(0);
		for(int i = 0; i < sellpdlist.size(); i++) {
			sellListModel.addRow(new String[] {sellpdlist.get(i).getSell_code(), 
					sellpdlist.get(i).getProduct_code(), 
					sellpdlist.get(i).getCompany_code(), ""+sellpdlist.get(i).getSell_quantity(),
					""+sellpdlist.get(i).getSell_amount(), sellpdlist.get(i).getOutput_date(), sellpdlist.get(i).getSell_info()});
		}
	}

	public void updateTable(VOSellProduct sellpd, int row) {
		sellListModel.removeRow(row);
		sellListModel.addRow(new String[] {sellpd.getSell_code(), 
				sellpd.getProduct_code(), sellpd.getCompany_code(),""+sellpd.getSell_quantity(), ""+sellpd.getSell_amount(), sellpd.getOutput_date(), sellpd.getSell_info()});
	}

...
public class InViewInsertDe extends JFrame {
	...
	public InViewInsertDe() {
		...
		lblDeQuantity.setBounds(30, 125, 88, 29);
		...
		btnPdInsert.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(pdCode.equals("") || ctmCode.equals("")) {
					JOptionPane.showMessageDialog(null, "물품 및 거래처를 선택해 주세요", "물품 및 거래처 선택",JOptionPane.INFORMATION_MESSAGE);
					return;
				}
				...
				try {
					result = new InViewDAO().insertData(pdCode, ctmCode, quantity, info);
					if(result > 0) {
				...
				try {
					result = new InViewDAO().reflectStock(pdCode, quantity);
					if(result < 0) {
		...
		btnDeSet.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				new InDeViewSelectPDAndCtm().settable();
				dispose();
			}
		});

...
public class InViewToday extends JFrame {
	
	private JTable tableDeList;
	private JScrollPane scrollPaneDeList;
	private DefaultTableModel inListModel;
	public InViewToday() {
		...
		String[] incol = new String[] { "입고코드","거래품목","거래처","수량","총액","날짜"};

		inListModel = new DefaultTableModel(inrow, incol);
		tableDeList = new JTable(inListModel);

		scrollPaneDeList = new JScrollPane(tableDeList);
		scrollPaneDeList.setBounds(14, 109, 392, 201);
		getContentPane().add(scrollPaneDeList);

		JLabel lblNewLabel = new JLabel("금일 입고 내역");
		...
		btnNewButton.setBounds(142, 337, 138, 42);
		...
		try {
			ArrayList<VOInProduct> inList = new InViewDAO().setTodayIn();
			setTable(inList);
	...
	public void setTable(ArrayList<VOInProduct> inList) {
		inListModel.setRowCount(0);
		for(int i = 0; i < inList.size(); i++) {
			inListModel.addRow(new String[] {inList.get(i).getin_code(), 
					inList.get(i).getProduct_code(), 
					inList.get(i).getCompany_code(), ""+inList.get(i).getin_quantity(),
					""+inList.get(i).getin_amount(), inList.get(i).getOutput_date()});
		}
	}

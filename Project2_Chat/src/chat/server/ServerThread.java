...
public class ServerThread implements Runnable {
	...
	public synchronized void dataParsing(String data) {
		...
		case User.ECHO01: // 대기실 에코
			msg = token.nextToken();
			//String nick = new DBLogin().selectNick(user.getId());
		...
		case User.WHISPER: // 귓속말
			id = token.nextToken();
			//nick = new DBLogin().selectNick(user.getId());
			msg = token.nextToken();
			whisper(id, msg);
			break;
		}
		...
		for (int i = 0; i < userArray.size(); i++) {
			if (id.equals(userArray.get(i).getId())) {
				...
				try {
					...
					System.out.println("user.toPro : " + user.toProtocol());
					jta.append("성공 : 귓속말보냄 : " + user.toString() + "가 " + userArray.get(i).toString() + "에게" + "\n");
					System.out.println("user.toPro : " + user.toString());
					System.out.println("user.toPro : " + userArray.get(i).toString());
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
	...
	private void logout() {
		...
		for (int i = 0; i < userArray.size(); i++) {
			if (user.getId().equals(userArray.get(i).getId())) {
				System.out.println(userArray.get(i).getId() + "로그아웃");
				userArray.remove(i);
			}
		}
